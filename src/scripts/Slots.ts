...
export class Slots extends Phaser.GameObjects.Container {
    ...
    constructor(scene: Phaser.Scene, uiContainer: UiContainer, callback: () => void, SoundManager : SoundManager) {
        ...
        this.spacingX = this.symbolWidth * 6.5; // Add some spacing
        this.spacingY = this.symbolHeight * 5.4; // Add some spacing
...
class Symbols {
    ...
    stopAnimation() {
        ...
        if (this.bouncingTween) {
            this.bouncingTween.stop();
            this.symbol.setScale(1); // Reset scale when stopping
        }
    }
    endTween() {
        if (this.index.y < 3) {
